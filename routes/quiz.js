...
router.post("/", verifyAdmin, async (req, res) => {
  try {
    ...
    if (batchConfiguration) {
      if (
        !Array.isArray(batchConfiguration) ||
        batchConfiguration.length !== expectedCount
      ) {
        return res.status(400).json({
          success: false,
    ...
    const quizQuestionSets = questionSetCombination.map((setId, index) => {
      ...
      const batchConfig = batchConfiguration?.find(
        (bc) => bc.questionSetId === setId,
      );
      ...
      if (questionSet.usesBatches && batchNumber) {
        const batch = questionSet.batches.find(
          (b) => b.batchNumber === batchNumber && b.isActive,
        );

        if (!batch) {
          throw new Error(
            `Batch ${batchNumber} not found or inactive in question set "${questionSet.title}"`,
          );
        }
      ...
      // Legacy: If question set doesn't use batches
        ...
        throw new Error(
          `Question set "${questionSet.title}" uses batches but no batch number was specified`,
        );
      }
...
router.get("/slim", verifyAdmin, async (req, res) => {
  try {
    const Quiz = require("../models/Quiz");
    const quizzes = await Quiz.find({ isActive: true })
      .select("settings questionSets.questionSetId questionSets.order")
      .lean();

    // Rebuild questionSetCombination from questionSets
    const slim = quizzes.map((quiz) => ({
      _id: quiz._id,
      settings: quiz.settings,
      questionSetCombination:
        quiz.questionSets?.map((qs) => qs.questionSetId) ?? [],
    }));

    res.json({ success: true, quizzes: slim });
  } catch (error) {
  ...
  try {
    const quiz = await Quiz.findById(req.params.id)
      ...
      .populate(
        ...
        "title questionCount totalPoints usesBatches",
      );
...
router.put("/:id/question-sets", verifyAdmin, async (req, res) => {
  try {
    ...
    if (
      ...
      questionSetIds.length !== expectedCount
    ) {
      return res.status(400).json({
        success: false,
        message: `Exactly ${expectedCount} question set IDs are required`,
...
// @access  Private (Admin only)
router.get(
  "/by-combination/:setId1/:setId2/:setId3/:setId4",
  verifyAdmin,
  async (req, res) => {
    try {
      ...
      const exactMatches = quizzes.filter(
        (quiz) =>
          quiz.questionSetCombination.length === 4 &&
          combination.every((id) =>
            quiz.questionSetCombination.some((qsId) => qsId.toString() === id),
          ),
      );
