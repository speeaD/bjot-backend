...
router.get("/quiz/:quizId", verifyQuizTaker, async (req, res) => {
  try {
    ...
    const selectedQuestionSetOrder =
      assignedQuiz.questionSetOrder.length > 0
...
router.post("/quiz/:quizId/submit", verifyQuizTaker, async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        const questionSet = quiz.questionSets.find(
          (qs) => qs.orderNum === questionSetOrder,
        );

        if (!questionSet) {
          throw new Error(
            `Question set with order ${questionSetOrder} not found`,
          );
        }
        ...
        for (const answerData of answers) {
          ...
          if (!question) {
            console.warn(
              `Question ${answerData.questionId} not found in question set`,
            );
            continue;
          }
          ...
          const pointsAwarded =
            gradeResult.isCorrect === true ? question.points : 0;
    ...
    res.json({
      success: true,
      message: result.isFinalSubmission
